...
pkg_installed() {
  ...
  if command -v "${pkgIn}" &>/dev/null; then
    ...
    return 0
  elif yay -Ss "${pkgIn}" &>/dev/null; then
    return 0
  else
    return 1
  fi
...
take_screenshot() {
  ...
  if "grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
    if ! "${annotation_tool}" "${annotation_args[@]}"; then
      notify-send -a "Hyprland" "Screenshot Error" "Failed to open annotation tool"
      return 1
    fi
  else
    notify-send -a "Hyprland" "Screenshot Error" "Failed to take screenshot"
    return 1
  fi
}
...
sc) #? 󱉶 Use 'tesseract' to scan image then add to clipboard
  ...
  if ! GEOM=$(slurp); then
    notify-send -a "Hyprland" "OCR preview: Invalid geometry" -e -i "dialog-error"
    exit 1
  fi
  ...
  notify-send -a "Hyprland" "OCR preview" -i "${temp_screenshot}" -e
...
if [ -f "${save_dir}/${save_file}" ]; then
  notify-send -a "Hyprland" -i "${save_dir}/${save_file}" "saved in ${save_dir}"
fi
