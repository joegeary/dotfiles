...
STATE_FILE="~/.local/state/zoomstatus.json"

if ! command -v openhue &>/dev/null; then
  echo "{\"text\": \"Zoom: ERROR\", \"tooltip\": \"openhue utility is missing\"}"
  exit 1
fi

if [ ! -e "/home/joe/.openhue/config.yaml" ]; then
  echo "{\"text\": \"Zoom: ERROR\", \"tooltip\": \"openhue utility is not configured\"}"
  exit 1
fi
...
check() {
  if [ "$(xdotool search --name $ZOOM_WINDOW_TITLE)" ]; then
    ...
    #   set_onair
    # fi
    echo "{\"text\": \"󰊻\", \"tooltip\": \"Zoom - ON AIR\", \"class\": \"onair\"}"
  else
    # if [ -e $STATE_FILE ]; then
    #   set_offair
    # fi
    echo "{\"text\": \"󰊻\", \"tooltip\": \"Zoom - OFF AIR\", \"class\": \"\"}"
  fi
}
