#!/usr/bin/env bash
set -eo pipefail
source "/home/joe/.local/bin/__lib_taskwarrior.sh"
...
get_review_prs() {
  # PRs where you're invovled (author, assignee, commenter)
  local involved_prs
  involved_prs=$(gh search prs --involves joegeary --state open --limit 100 \
    --json title,url,number,repository,createdAt,updatedAt)

  # PRs where you're mentioned
  local mentioned_prs
  mentioned_prs=$(gh search prs --mentions joegeary --state open --limit 100 \
    --json title,url,number,repository,createdAt,updatedAt)

  # PRs where you're requested to review
  local review_prs
  review_prs=$(gh search prs --review-requested joegeary --state open --limit 100 \
    --json title,url,number,repository,createdAt,updatedAt)

  # Combine all results and remove dupes
...
main() {
  ...
  # Create new tasks for PRs or update exisiting ones
  while read -r pr; do
