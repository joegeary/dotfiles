...
def write_report(projects, summaries):
    ...
    for i, (proj, session) in enumerate(costly, 1):
        lines.append(
            f"### {i}. {proj} — {format_tokens(session['total_tokens'])} tokens"
        )
        ...
        u = session["usage"]
        lines.append(
            f"- **Tokens**: input={format_tokens(u.get('input_tokens', 0))}, cache_create={format_tokens(u.get('cache_creation_input_tokens', 0))}, cache_read={format_tokens(u.get('cache_read_input_tokens', 0))}, output={format_tokens(u.get('output_tokens', 0))}"
        )
    ...
    for proj_name, sessions in projects.items():
        sub_tokens = sum(
            sub["total_tokens"] for s in sessions for sub in s["subagent_sessions"]
        )
...
def print_summary(summaries, projects):
    ...
    for proj, session in find_costly_sessions(projects, top_n=10):
        ...
        print(
            f"  [{ts}] {proj}: {format_tokens(session['total_tokens'])} — {first_prompt}"
        )
